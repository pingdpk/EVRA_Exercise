...
public class StopTransactionHandler implements OcppActionHandler {
    ...
    public StopTransactionHandler(ChargingCommandService chargingCommandService, OcppCommandMapper mapper, OcppFrames frames) {
    ...
    public String handle(String chargerId, OcppCallFrame frame) {
        CommandResult result = chargingCommandService.handle(mapper.stopTransaction(chargerId, frame.getPayload(), frame.getUniqueId()));
        return frames.callResult(frame.getUniqueId(), Map.of("idTagInfo", Map.of("status", "Accepted"), "status", result.getStatus(), "message", result.getMessage()));
    }
}
