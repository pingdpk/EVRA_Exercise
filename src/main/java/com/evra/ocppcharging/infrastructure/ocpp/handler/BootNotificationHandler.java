...
public class BootNotificationHandler implements OcppActionHandler {
    ...
    public BootNotificationHandler(ChargingCommandService chargingCommandService, OcppCommandMapper mapper, OcppFrames frames) {
    ...
    public String handle(String chargerId, OcppCallFrame frame) {
        CommandResult result = chargingCommandService.handle(mapper.bootNotification(chargerId, frame.getPayload(), frame.getUniqueId()));
        return frames.callResult(frame.getUniqueId(), Map.of("status", "Accepted", "currentTime", java.time.Instant.now().toString(), "interval", 300));
    }
}
