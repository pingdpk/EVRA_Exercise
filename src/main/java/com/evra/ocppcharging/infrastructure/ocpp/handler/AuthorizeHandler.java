...
public class AuthorizeHandler implements OcppActionHandler {
    ...
    public AuthorizeHandler(ChargingCommandService chargingCommandService, OcppCommandMapper mapper, OcppFrames frames) {
    ...
    public String handle(String chargerId, OcppCallFrame frame) {
        CommandResult result = chargingCommandService.handle(mapper.authorize(chargerId, frame.getPayload(), frame.getUniqueId()));
        return frames.callResult(frame.getUniqueId(), Map.of("idTagInfo", Map.of("status", "Accepted"), "message", result.getMessage()));
    }
}
