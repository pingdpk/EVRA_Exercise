...
public class MeterValuesHandler implements OcppActionHandler {
    ...
    public MeterValuesHandler(ChargingCommandService chargingCommandService, OcppCommandMapper mapper, OcppFrames frames) {
    ...
    public String handle(String chargerId, OcppCallFrame frame) {
        CommandResult result = chargingCommandService.handle(mapper.meterValues(chargerId, frame.getPayload(), frame.getUniqueId()));
        return frames.callResult(frame.getUniqueId(), Map.of("status", result.getStatus(), "message", result.getMessage()));
    }
}
